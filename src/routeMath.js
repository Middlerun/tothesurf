...
/*export function generateCachedDistances() {
  let total = 0
  for (let i = 0; i < route.length; i++) {
    if (i > 0) {
      total += distance(route[i - 1], route[i])
    }
    console.log(`{ longitude: ${route[i].longitude}, latitude: ${route[i].latitude}, distance: ${total} },`)
  }
}*/
...
export function distanceAlongRoute(position) {
  ...
  /*console.log(nearestSegmentIndex)
  console.log(partialRouteDistance(route.slice(0, nearestSegmentIndex+1)))
  console.log(distanceAlongSegment(
